            ...
            <div className='flex justify-center items-center'>
                {
                    (NODE_ENV === "development") ?
                        <button id='register-btn' className='p-3 pr-5 pl-5 text-lg rounded mr-5' style={{ backgroundColor: 'var(--btn-color)' }} onClick={() => { navigate("/register"); }}>Register</button>
                    : null
                }

                <button id='login-btn' className='p-3 pr-5 pl-5 text-lg rounded mr-5' style={{ backgroundColor: 'var(--btn-two-color)' }} onClick={() => { navigate("/login"); }}>  &nbsp;Login&nbsp;</button>
            </div>
        </div>
    );
}
